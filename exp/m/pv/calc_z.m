function Z = calc_z(latitudeinput, longitudeinput, Timezoneinput, DSTinput, heurelocaleinput, jour, mois, annee)
% calc_z(34.88,-1.31, 1, 0, 14, 8,6,2016 )
...
function angle =  heure2rd(  heure)
	angle = heure*pi/12.0;
end
    function heure= rd2heure( angle)
	heure = angle*12.0/pi;
end
%%-------------------------------------------------------------------------
...
% iteration bis
	...
	%fprintf("T=%g JC à %s\n",T, h2hminsec(Z_GMT, SS));
	Soleil.L=SoleilLongitude(T);
    [Soleil.RA, Soleil.delta] = eclipticL2RAndelta( T, Soleil);
	theta0=TempsSideral( T);
	GMST0=theta0-heure2rd(Z_GMT);
	Z=TimeZone+rd2heure(Soleil.RA-GMST0-longitude);    % on le prend avec TimeZone
	if (Z<0) 
        Z=Z+24;
    end
end